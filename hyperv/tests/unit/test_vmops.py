...
class VMOpsTestCase(test_base.HyperVBaseTestCase):
    ...
    @mock.patch('hyperv.nova.vmops.VMOps.check_vm_image_type')
    @mock.patch('hyperv.nova.vmops.VMOps._create_root_vhd',
                return_value=mock.sentinel.VHD_PATH)
    @mock.patch('hyperv.nova.vmops.VMOps._create_root_iso')
    def _test_create_root_device(self, mock_create_root_iso,
                                 mock_create_root_device,
                                 mock_check_vm_image_type,
                                 fake_disk_type):
        mock_instance = fake_instance.fake_instance_obj(self.context)
        mock_root_disk_info = {'type': fake_disk_type}

        self._vmops._create_root_device(self.context, mock_instance,
            mock_root_disk_info, mock.sentinel.VM_GEN_1)
        if fake_disk_type == constants.DISK:
    ...
    def test_create_root_device_type_disk(self):
        self._test_create_root_device(fake_disk_type=constants.DISK)

    def test_create_root_device_type_iso(self):
        self._test_create_root_device(fake_disk_type=constants.DVD)

    def test_create_root_iso(self):
        mock_instance = fake_instance.fake_instance_obj(self.context)

        self._vmops._imagecache.get_cached_image = mock.MagicMock(
            return_value=mock.sentinel.CACHED_ISO_PATH)
        mock_get_cached_image = self._vmops._imagecache.get_cached_image

        self._vmops._pathutils.get_root_vhd_path = mock.MagicMock(
            return_value=mock.sentinel.ROOT_ISO_PATH)
        mock_get_root_vhd_path = self._vmops._pathutils.get_root_vhd_path

        self._vmops._pathutils.copyfile = mock.MagicMock()
    ...
    @mock.patch('hyperv.nova.vmops.VMOps._create_ephemeral_disk')
    def test_create_ephemerals(self, mock_create_ephemeral_disk):
        mock_instance = fake_instance.fake_instance_obj(self.context)

        fake_ephemerals = [dict(), dict()]
        self._vmops._pathutils.get_instance_dir = mock.MagicMock(
            return_value='fake_instance_dir')
        mock_get_instance_dir = self._vmops._pathutils.get_instance_dir
        self._vmops._vhdutils.get_best_supported_vhd_format = mock.MagicMock(
            return_value='vhd')

        self._vmops._create_ephemerals(mock_instance, fake_ephemerals)

        mock_get_instance_dir.assert_has_calls([mock.call(mock_instance.name),
        ...
        self.assertEqual(fake_ephemerals[0]['path'],
                         os.path.join('fake_instance_dir', 'eph0'))
        self.assertEqual(fake_ephemerals[1]['path'],
                         os.path.join('fake_instance_dir', 'eph1'))
    ...
    def test_create_ephemeral_disk(self):
        ...
        self._vmops._vhdutils.create_dynamic_vhd = mock.MagicMock()
        mock_create_dynamic_vhd = self._vmops._vhdutils.create_dynamic_vhd

        self._vmops._create_ephemeral_disk(mock_instance.name,
            mock_ephemeral_info)

        mock_create_dynamic_vhd.assert_called_once_with('fake_eph_path.vhd',
            10 * units.Gi, 'vhd')
    ...
    @mock.patch('hyperv.nova.vif.get_vif_driver')
    @mock.patch('hyperv.nova.block_device_manager.BlockDeviceInfoManager.get_root_device')
    @mock.patch('hyperv.nova.block_device_manager.BlockDeviceInfoManager.validate_and_update_bdi')
    def _test_spawn(self, mock_validate_and_update_bdi, mock_get_root_device,
        ...
        mock_get_root_device.return_value = root_device_info
        ...
        if exists:
            ...
            mock_get_root_device.assert_called_once_with(fake_vm_gen,
                mock_image_meta, block_device_info)
            mock_create_root_device.assert_called_once_with(self.context,
                                                            mock_instance,
                                                            root_device_info,
                                                            fake_vm_gen)
    ...
    @mock.patch.object(vmops.VMOps, '_attach_volumes')
    ...
    def _test_create_instance(self, mock_configure_remotefx, mock_create_pipes,
        ...
        if remotefx is True and vm_gen == constants.VM_GEN_2:
            ...
            mock_attach_ephemerals.assert_called_once_with(mock_instance.name,
                block_device_info['ephemerals'])
            mock_attach_volumes.assert_called_once_with(mock_instance.name,
    ...
    def test_create_instance(self):
        ...
        self._test_create_instance(root_device_info=root_device_info,
                                   block_device_info=block_device_info,
    ...
    @mock.patch('hyperv.nova.vmops.VMOps._attach_drive')
    def _test_attach_root_device(self, mock_attach_drive, root_device_info):
        mock_instance = fake_instance.fake_instance_obj(self.context)

        self._vmops._volumeops.attach_volume = mock.MagicMock()

        self._vmops._attach_root_device(mock_instance.name, root_device_info)

        if root_device_info['type'] == constants.VOLUME:
            self._vmops._volumeops.attach_volume.assert_called_once_with(
                root_device_info['connection_info'], mock_instance.name,
                ebs_root=True)
        else:
            mock_attach_drive.assert_called_with(mock_instance.name,
                root_device_info['path'], 0, root_device_info['boot_index'],
    ...
    def test_attach_root_device_volume(self):
        ...
        self._test_attach_root_device(root_device_info=root_device_info)

    def test_attach_root_device_disk(self):
        ...
        mock_attach_drive.assert_has_calls(
            [mock.call(mock_instance.name, mock.sentinel.PATH1, 0,
             ...
             mock.call(mock_instance.name, mock.sentinel.PATH2, 0,
                       2, constants.CTRL_TYPE_SCSI, constants.DISK)
            ])
    ...
    @mock.patch('nova.virt.configdrive.required_by')
    @mock.patch.object(vmops.VMOps, '_create_root_device')
    ...
    def test_rescue_instance(self, mock_power_on,
        ...
        mock_create_root_device.return_value = mock.sentinel.rescue_vhd_path
