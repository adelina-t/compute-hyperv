...
class ImageCache(imagecache.ImageCacheManager):
    ...
    def _update_image_timestamp(self, image):
        ...
        for img in backing_files['base_file']:
            os.utime(img, None)
        for img in backing_files['resized_files']:
            os.utime(img, None)
    ...
    def _get_image_backing_files(self, image):
        ...
        base_file.append(self._pathutils.get_image_path(image))
        resize_re = re.compile('%s_[0-9]+$' % image)
        for img in self.unexplained_images:
            match = resize_re.match(img)
            if match:
                resized_files.append(self._pathutils.get_image_path(img))

        return {'base_file': base_file,
